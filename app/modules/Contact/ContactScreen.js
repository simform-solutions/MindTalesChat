...
import { FlatList, NativeModules, View, Image, Text, TouchableOpacity } from 'react-native';
...
const ContactScreen = () => {
    ...
    return (
        <Container>
            <CustomHeader left title={'Select Contact'} leftIcon={icons.back} leftOnPress={()=>navigation.goBack()}/>
            <View style={styles.whiteContainer}>
                {errors || contactsList.length === 0 ? (
                    <View style={styles.whiteContainerCenter}> 
                     <Text style={styles.textStyle}>{errors ? errors : 'Please add contacts'}</Text> 
                     </View>
                ) : (
                    <FlatList
                        data={contactsList}
